...
# 상자 이미지 로딩
blue_box_image = pygame.image.load('Image/BLUE.png')
yellow_box_image = pygame.image.load('Image/YELLOW.png')
special_box_image = pygame.image.load('Image/YELLOW.png')

game_map[4] = game_map[4][:4] + 'B' + game_map[4][5:]
...
class Player(pygame.sprite.Sprite): # player setting
    def __init__(self, pos, group):
        ...
        self.image = pygame.image.load('Image/시나.png')
    ...
    def can_move(self, new_position):
        # 플레이어의 게임 맵 내 그리드 위치 계산
        grid_x = int((new_position.x + self.rect.width / 2) // 128)
        grid_y = int((new_position.y + self.rect.height / 2) // 128)
        
        # 새 위치가 게임 맵 내의 파란 상자와 충돌하는지 확인
        if game_map[grid_y][grid_x] == 'T':
            return False
        
        # 파란 박스와의 충돌 여부 확인
        player_rect = pygame.Rect(new_position.x, new_position.y, self.rect.width, self.rect.height)
        for y, row in enumerate(game_map):
            for x, char in enumerate(row):
                if char == 'T':
                    box_rect = pygame.Rect(x * 128, y * 128, 128, 128)
    ...
    def update(self):
        ...
        # Calculate smoothed interpolation factor using a smootherstep function
        t = min(1, max(0, (pygame.time.get_ticks() - self.last_update_time) / self.speed))
        t_smooth = t * t * t * (t * (t * 6 - 15) + 10)  # Smootherstep function

        # Calculate interpolated position using Vector2 lerp function
        interpolated_position = pygame.math.Vector2(self.rect.center).lerp(new_position, t_smooth)

        # 새 위치가 배경 경계 내에 있는지 확인
        within_x_boundary = 0 <= interpolated_position.x <= camera_group.ground_rect.width
        within_y_boundary = 0 <= interpolated_position.y <= camera_group.ground_rect.height

        if within_x_boundary and within_y_boundary and self.can_move(interpolated_position):
            self.rect.center = interpolated_position
...
class Camera(pygame.sprite.Group): # camera setting
    ...
    def center_target_camera(self, target):
        ...
        self.offset.x = max(0, min(self.offset.x, len(game_map[0]) * 128 - self.display_surface.get_width()))
        self.offset.y = max(0, min(self.offset.y, len(game_map) * 128 - self.display_surface.get_height()))
    ...
    def custom_draw(self, player):
        ...
        for y, row in enumerate(game_map):
            for x, char in enumerate(row):
                box_image = None
                if char == 'T':
                    box_image = blue_box_image
                elif char == '.':
                    box_image = yellow_box_image
                elif char == 'E':
                    box_image = special_box_image
                elif char == 'B':  # Add this condition for the couple1_button
                    couple1_button.draw(screen)


                if box_image is not None:
                    box_rect = box_image.get_rect(topleft=(x * 128 - self.offset.x, y * 128 - self.offset.y))
...
camera_group = Camera()
player = Player((320, 360), camera_group)
...
countdown_duration = 1000 # 쉬는 시간 시간 조절 기능(점심 시간은 # 점심 시간 조절 검색해서 바꿀 것))
